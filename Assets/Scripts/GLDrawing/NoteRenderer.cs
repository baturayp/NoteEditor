        ...
        void LateUpdate()
        {
            ...
            foreach (var noteObj in EditData.Notes.Values)
            {
                ...
                if (min <= canvasPosOfNote.x && canvasPosOfNote.x <= max)
                {
                    ...
                    if (noteObj.note.type == Notes.NoteTypes.Multi && noteObj.note.howManyTimes > 0)
                    {
                        GLQuadDrawer.Draw(new Geometry(
                        new[] {
                            new Vector3(screenPos.x, screenPos.y - (noteObj.note.howManyTimes + drawSize), 0),
                            new Vector3(screenPos.x + (noteObj.note.howManyTimes + drawSize), screenPos.y, 0),
                            new Vector3(screenPos.x, screenPos.y + (noteObj.note.howManyTimes + drawSize), 0),
                            new Vector3(screenPos.x - (noteObj.note.howManyTimes + drawSize), screenPos.y, 0)
                        },
                        noteObj.NoteColor));
