    ...
    public class EditNotesPresenter : SingletonMonoBehaviour<EditNotesPresenter>
    {
        ...
        void Init()
        {
            ...
            closestNoteAreaOnMouseDownObservable
                ...
                .Merge(closestNoteAreaOnMouseDownObservable
                    .Where(_ => EditState.NoteType.Value == NoteTypes.Multi))
                .Subscribe(_ =>
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (EditState.LongNoteTailPosition.Value.block == NoteCanvas.ClosestNotePosition.Value.block && NoteCanvas.ClosestNotePosition.Value.num > EditState.LongNoteTailPosition.Value.num)
                            {
            ...
            this.UpdateAsObservable()
                .Where(_ => EditData.Notes.ContainsKey(NoteCanvas.ClosestNotePosition.Value))
                .Where(_ => Input.GetMouseButtonDown(1))
                .Select(closestNote => NoteCanvas.ClosestNotePosition.Value)
                .Subscribe(closestNote => RequestForRemoveNote.OnNext(EditData.Notes[closestNote].note));
            ...
            closestNoteAreaOnMouseDownObservable
                ...
                .Where(_ => KeyInput.ShiftKey())
                .Where(_ => EditData.Notes.ContainsKey(NoteCanvas.ClosestNotePosition.Value))
                .Where(_ => EditData.Notes[NoteCanvas.ClosestNotePosition.Value].note.type == NoteTypes.Long)
                .Do(_ => EditState.NoteType.Value = NoteTypes.Long)
                .Subscribe(_ => EditData.Notes[NoteCanvas.ClosestNotePosition.Value].OnClickObservable.OnNext(Unit.Default));
            ...
            finishEditLongNoteObservable.Subscribe(_ => {
                if (EditData.Notes.ContainsKey(EditState.LongNoteTailPosition.Value))
                {
                    var currentTailNote = EditData.Notes[EditState.LongNoteTailPosition.Value].note;
                    RequestForRemoveNote.OnNext(currentTailNote);
                }
            ...
            RequestForEditNote.Subscribe(note =>
            {
                ...
                else if (note.type == NoteTypes.Multi)
                {
                    if (!EditData.Notes.ContainsKey(note.position))
                    {
                        RequestForAddNote.OnNext(note);
